...
struct vec_u64x2 {
	...
	explicit vec_u64x2(const uint8_t *inp) {
	...
	explicit vec_u64x2( bool b ) { uint64_t b0=b; uint64_t b1=0; b1 -= b0; for(unsigned i=0;i<_num_limb;i++)v[i]=b1; }
...
struct mat_u64{
	...
	vec_v prod( const vec_h & a_ ) const {
		vec_v r;
		r.set_zero();
		for(unsigned i=0;i<N_VEC;i++) { vec_v tmp(a_.get_ele(i)); tmp &= v[i]; r ^= tmp; }
		return r;
	}
