...
struct quartz_key<ext,129,minus,vinegar>{
	...
	void get_central_poly( poly_t & r_poly , uint32_t v ) const {
		...
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_128[i];
		// { vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_128[i]; cc ^= *(gf_t*)&tt; }
		//{ cc ^= ((1<<i)&v)?l_b_128[i]:gf_t::zero(); }
		...
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_64[i];
		// { vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_64[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_64[i]:gf_t::zero();
		...
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_32[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_32[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_32[i]:gf_t::zero();
		...
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_16[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_16[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_16[i]:gf_t::zero();
		r_poly.append_term(cc,16);

		r_poly.append_term(q_a_8[2],12);
		r_poly.append_term(q_a_8[1],10);
		r_poly.append_term(q_a_8[0],9);
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_8[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_8[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_8[i]:gf_t::zero();
		r_poly.append_term(cc,8);

		r_poly.append_term(q_a_4[1],6);
		r_poly.append_term(q_a_4[0],5);
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_4[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_4[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_4[i]:gf_t::zero();
		r_poly.append_term(cc,4);

		r_poly.append_term(q_a_2[0],3);
		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_2[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_2[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_2[i]:gf_t::zero();
		r_poly.append_term(cc,2);

		cc.set_zero(); for(unsigned i=0;i<vinegar;i++) cc ^= gf_t(bool((1<<i)&v)) & l_b_1[i];
		//{ vec_t_t tt(bool((1<<i)&v)); tt ^= *(const vec_t_t *)&l_b_1[i]; cc ^= *(gf_t*)&tt; }
		//cc ^= ((1<<i)&v)?l_b_1[i]:gf_t::zero();
		...
		for(unsigned i=0;i<vinegar;i++) {
			...
			for(unsigned j=0;j<vinegar;j++) {
				bool add2 = (1<<j) & v;
				cc ^= gf_t(bool( add & add2 )) & c_r[i*vinegar+j];
				//vec_t_t tt(bool(add&add2));
				//tt ^= *(const vec_t_t *)&c_r[i*vinegar+j];
				//cc ^= *(gf_t*)&tt;
...
template <unsigned ext,unsigned minus,unsigned vinegar>
struct quartz_key<ext,5,minus,vinegar>{
	//static const unsigned ext = 96;
	static const unsigned max_d = 5;
	//static const unsigned minus = 6;
	//static const unsigned vinegar = 6;
	...
	static unsigned num_byte() { return mat_t_t::num_byte()+vec_t_t::num_byte()+mat_s_t::num_byte()+vec_s_t::num_byte()
		+gf_t::num_byte()*(2+ (3+vinegar)*vinegar);}
	void dump( uint8_t * m ) const {
		t_mat.dump(m); m += mat_t_t::num_byte();
		t_vec.dump(m); m += vec_t_t::num_byte();
		s_mat.dump(m); m += mat_s_t::num_byte();
		s_vec.dump(m); m += vec_s_t::num_byte();
		q_a_2[0].dump(m); m += gf_t::num_byte();
		q_a_4[0].dump(m); m += gf_t::num_byte();
	...
	void set( const uint8_t * m ) {
		...
		q_a_2[0] = gf_t(m); m += gf_t::num_byte();
		q_a_4[0] = gf_t(m); m += gf_t::num_byte();
...
//void quartz_pubmap_seckey( VEC<ext-minus>& z , const quartz_key<ext,max_d,minus,vinegar> & sk , const VEC<ext+vinegar> & w )
{
	...
	static unsigned char tmp[32] __attribute__((aligned(32))) = {0};
	yy.dump( tmp );
	VEC<ext> y( tmp );
